...
MODEL_REFRESH_DAYS = [
    CheckHoliday(x) for x in pd.date_range(START_DATE, END_DATE, freq="BMS")
]
MODEL_REFRESH_DAYS = list(
    np.random.choice(
        MODEL_REFRESH_DAYS, int(len(MODEL_REFRESH_DAYS) / 3), replace=False
    )
)
########## END PARAMETERS #############
...
def TradingDay(
    current_day, portfolio, buying_power, api, scalers, model=None, cov_matrix=None
):
    if (current_day in MODEL_REFRESH_DAYS) or (model is None):
        ...
        model, scalers = TrainEncoderModel(
        ...
        data = GetHistoricalData(
            stocks_to_predict, api, day, n_data_points=HISTORY_STEPS
        )
    ...
    for i, col in enumerate(data.columns):
        scaler = scalers[col]
        predictions[:, i] = scaler.inverse_transform(
...
while day < end_day:
    ...
    preds, orders, prices, model, scalers, cov_matrix = TradingDay(
        day, portfolio, buying_power, api, scalers, model, cov_matrix
    )
...
baseline = GetLongReturns(
    IdentifyStocksOfInterest(method="handpick"), api, START_DATE, END_DATE
)
