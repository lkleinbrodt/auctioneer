...
api = alpaca.REST(key_id=alpaca_api['APCA_API_KEY_ID'], secret_key = alpaca_api['APCA_API_SECRET_KEY'], base_url=alpaca_api['APCA_API_BASE_URL'])
...
historical_data = pd.concat([api.get_barset(symbols = symbols[:5], timeframe = TIME_STEP, after = start_date_str, until = end_date_str).df for symbols in soi_chunks], axis = 1)
...
first_trading_day = pd.Timestamp(api.get_calendar(start = start_date_str, end = start_date_str)[0].date, tz = 'America/New_York')
...
while min_date > first_trading_day:
    ...
    new_data = pd.concat([api.get_barset(symbols = symbols[:5], timeframe = TIME_STEP, limit = 1000, after = start_date_str, until = min_date.isoformat()).df for symbols in soi_chunks])
...
historical_data = api.get_barset(symbols = soi[:5], timeframe = '15Min', limit = 50, after = START_DATE, until = END_DATE).df
