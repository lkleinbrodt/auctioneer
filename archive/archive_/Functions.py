...
def GetHistoricalData(
    symbols, api, end_date=datetime.now(), n_data_points=7500, time_interval="hour"
):
    ...
    while data.shape[0] < n_data_points:
        ...
        for sym in symbols_to_pull:
            quotes = api.polygon.historic_agg_v2(
                symbol=sym,
                multiplier=1,
    ...
    if len(bad_cols) > 0:
        ...
        logging.debug(
            "Skipping {}, had missing values for the period.".format(list(bad_cols))
        )
...
def GetDayQuotes(symbols, api, date, open_or_close="open"):
    ...
    for sym in symbols:
        quotes = api.polygon.historic_agg_v2(
            symbol=sym, multiplier=1, timespan="day", _from=date, to=date
        ).df
...
def GetLongReturns(symbols_to_consider, api, start, end):
    ...
    valid_symbols = [
        ...
        if sym in starting_prices.keys() and ending_prices.keys()
    ]
    invalid_symbols = [sym for sym in symbols_to_consider if sym not in valid_symbols]

    if len(invalid_symbols) > 0:
        logging.debug(
            "Skipping {}, had missing values for the period.".format(invalid_symbols)
        )
...
def GenerateWindowData(df, HISTORY_STEPS, TARGET_STEPS, train_test_split=0.9):
...
def TrainEncoderModel(
    df, HISTORY_STEPS, TARGET_STEPS, MAX_EPOCHS, BATCH_SIZE, LEARNING_RATE
):
    X_train, Y_train, X_test, Y_test, scalers = GenerateWindowData(
    ...
    model_checkpoints = tf.keras.callbacks.ModelCheckpoint(
        "models/checkpoints/", save_best_only=True, save_weights_only=True
    )
