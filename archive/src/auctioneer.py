...
class Portfolio:
    ...
    def value(self, date=None):
        ...
        out = {"Cash": self.balance, "AUM": aum, "Total": self.balance + aum}
            ...
            for t in transaction_log:
                if t["symbol"] == symbol:
                    if t["action"] == "buy":
                        ...
                        color = "red"
                    plt.axvline(t["date"], color=color, alpha=0.05)
...
def pull_crypto_prices(
    symbols, start_date, end_date=None, timeframe="minute", client=None, column="close"
):
    if client is None:
        client = CryptoHistoricalDataClient()
...
def window_data(df, history_steps, target_steps, train_test_split=0.9):
...
def download_s3_directory(s3, s3_directory, local_directory):
    ...
    for obj in objects:
        ...
        s3.download_file(
            S3_BUCKET, obj["Key"], local_directory + obj["Key"]
        )  # save to same path
